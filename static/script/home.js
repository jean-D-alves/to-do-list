...
async function getTacks() {
  try {
    const DoneTrue = await fetch("http://localhost:3000/tacks/?done=true");
    const Donefalse = await fetch("http://localhost:3000/tacks/?done=false");
          ...
          <tr class="donetrue">
            ...
            <td>
              <button class="donetruebtn"onclick="taskdone('${t.id}',${t.done})" id="${t.id}">
                <img
                  class="icon"
                  src="/static/assest/verificar.svg"
                  alt=""
                /></button
              ><button class="donetruebtn" onclick="editask('${t.id}')">
                <img
                  class="icon"
                  src="/static/assest/lapis.svg"
                  alt=""
                /></button
              ><button class="donetruebtn" onclick="deletetask('${t.id}')">
                <img class="icon" src="/static/assest/lixo.svg" alt="" />
              </button>
            </td>
          </tr>`
      )
...
async function deletetask(id) {
  try {
    const response = await fetch(`http://localhost:3000/tacks/${id}`, {
      method: "DELETE",
    });
    getTacks();
  } catch (err) {
...
async function editask(id) {
  ...
  try {
    const editresponse = await fetch(`http://localhost:3000/tacks/${id}`, {
      method: "PATCH",
      headers: {
...
async function taskdone(id, done) {
  if (done === false) {
    try {
      const responsedone = await fetch(`http://localhost:3000/tacks/${id}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
